	...
	public unsafe static int Invoke(void* @this, [NativeType("class crnd::static_huffman_data_model const &")] void* model)
	{
		unchecked
		{
			...
			if ((uint)num7 <= (uint)((crnd_prefix_coding_decoder_tables*)pDecode_tables)->m_table_max_code)
			{
				...
				if (num8 == -1)
				{
					crnd_assert.Invoke(String_fwybtzb.__pointer, String_yguirrd.__pointer, 3253);
				}
				num9 = num8 & 0xFFFF;
				i = num8 >>> 16;
				if ((byte)(*(sbyte*)vector_unsigned_char_Index_3lowkad.Invoke(&((crnd_static_huffman_data_model*)model)->m_code_sizes, num9)) != i)
				{
					crnd_assert.Invoke(String_e4i4x9a.__pointer, String_yguirrd.__pointer, 3257);
				}
			}
			else
			{
				...
				{
					crnd_assert.Invoke(String_xzkwrkb.__pointer, String_yguirrd.__pointer, 3275);
					return 0;
				}
