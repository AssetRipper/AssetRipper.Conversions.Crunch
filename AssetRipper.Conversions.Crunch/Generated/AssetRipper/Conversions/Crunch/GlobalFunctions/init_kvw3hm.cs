...
[CleanName("init")]
internal static partial class init_kvw3hm
{
	...
	public unsafe static bool Invoke(void* @this, [NativeType("unsigned int")] int num_syms, [NativeType("unsigned char const *")] void* pCodesizes, [NativeType("unsigned int")] int table_bits)
	{
		...
		unchecked
		{
			...
			else
			{
				...
				zero_object_gms5dk.Invoke(&localsPointer->field_1);
				...
				for (int k = 0; (uint)k < (uint)num_syms; k++)
				{
					...
					if (num9 != 0)
					{
						...
						{
							crnd_assert.Invoke(String_ez6imn.__pointer, String_vvx8bt.__pointer, 2276);
						}
						int num10 = ((int*)(&localsPointer->field_2))[(uint)num9]++;
						if ((uint)num10 >= (uint)num4)
						{
							crnd_assert.Invoke(String_zzatd6.__pointer, String_vvx8bt.__pointer, 2280);
						}
				...
				if (num != 0)
				{
					...
					if ((uint)num11 > (uint)((crnd_prefix_coding_decoder_tables*)@this)->m_cur_lookup_size)
					{
						...
						void* lookup = crnd_new_array_zqdtgg.Invoke(num11);
						if (ExceptionInfo.Current != null)
						{
							return false;
						}
					...
					for (int l = 1; (uint)l <= (uint)num; l++)
					{
						...
						for (int m = num14; (uint)m <= (uint)num15; m++)
						{
							...
							{
								crnd_assert.Invoke(String_7anbbg.__pointer, String_vvx8bt.__pointer, 2322);
							}
							for (int n = 0; (uint)n < (uint)num13; n++)
							{
								...
								{
									crnd_assert.Invoke(String_g3mv3q.__pointer, String_vvx8bt.__pointer, 2328);
								}
								if (((int*)((crnd_prefix_coding_decoder_tables*)@this)->m_lookup)[(uint)num18] != -1)
								{
									crnd_assert.Invoke(String_5k7f4m.__pointer, String_vvx8bt.__pointer, 2330);
								}
